...
<link rel="stylesheet" href="{{ asset('css/myadv_details.css')}}">
<head>
      ...
      <nav id="breadcrumbs">
        <ul>
          ...
          <li>
            ...
            <a href="{{ route('dashboard') }}">آگهی های من</a>
          </li>
...
<div class="d-flex justify-content-center container mt-2 " style="justify-content: right!important;margin-top: 2.5rem !important;">
    ...
    <div class="col-lg-6 pt-0 bg-white div-details"
      ...
      <div class="stats">
        ...
        <p class="p1">{{$itemdetail->cat}}</p>
        <hr id="h">

        <p class="font-weight-bold">
          {{ __('توضیحات آگهی') }}
        </p>
        <p class="p1">{{ $itemdetail->description }}</p>
        <hr id="h">

        <p class="font-weight-bold">
          {{ __('توضیحات کالای مورد معاوضه') }}
        </p>
        <p class="p1">{{ $itemdetail->exchangedesc }}</p>
        <hr id="h">

        <p class="font-weight-bold">
          {{ __('دانشگاه') }}
        </p>
        <p class="p1">{{ $itemdetail->uni }}</p>
        <hr id="h">

        <p class="font-weight-bold">
          {{ __('شهر ') }}
        </p>
        <p class="p1">{{ $itemdetail->cname }}</p>
        <hr id="h">

        <p class="font-weight-bold">
          {{ __('راه ارتباطی') }}
        </p>
        <p class="p1">{{ $itemdetail->contactway }}</p>
      </div>
    </div>
...
<div class="h-scroll-wrapper">
  ...
  @foreach($orderable_items as $item)
